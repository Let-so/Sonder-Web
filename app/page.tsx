function Eyebrow({ children }: { children: React.ReactNode }) {
  return (
    <span className="inline-block rounded-full border border-white/15 px-3 py-1 text-[10px] text-white/70">
      {children}
    </span>
  );
}
...
function Step({ num, title, text }: { num: number; title: string; text: string }) {
  return (
    <li className="relative rounded-2xl border border-white/10 p-5"
        style={{ backdropFilter: "blur(10px)", background: "color-mix(in srgb, rgb(17 24 39), transparent 50%)" }}>
        ...
        <div className="mx-auto max-w-6xl px-4 grid lg:grid-cols-2 gap-8 items-center">
          <div>
            ...
            <div className="mt-5 flex flex-wrap items-center gap-2">
              ...
              <a href="#que-es" className="rounded-xl border border-white/15 px-4 py-2 text-sm">
                Saber más
              </a>
            </div>
          </div>
          ...
          <div>
            <div className="rounded-3xl border border-white/10 p-5 shadow-2xl max-w-sm mx-auto"
                 style={{ backdropFilter: "blur(10px)", background: "color-mix(in srgb, rgb(17 24 39), transparent 50%)" }}>
              ...
              <div className="rounded-2xl border border-white/10 p-3 mt-3">
                <div className="text-sm">Sertralina 50mg</div>
                <p className="mt-1 text-[11px] text-white/70">Recordatorio diario, 08:00</p>
              </div>
              <div className="rounded-2xl border border-white/10 p-3 mt-3">
                <div className="text-sm">Posible baja respuesta</div>
                <p className="mt-1 text-[11px] text-white/70">La IA detectó señales tempranas. Se notificó al médico.</p>
              </div>
              <div className="flex items-center gap-2 mt-3">
                <a className="w-full rounded-xl border border-white/15 px-3 py-2 text-center text-sm" href="#">
                  Subir estudio
                </a>
        ...
        <div className="mx-auto max-w-6xl px-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-[11px] text-white/60">© {new Date().getFullYear()} — Todos los derechos reservados</div>
            </div>
          </div>
          <div className="flex gap-5 text-sm text-white/70">
            ...
            <a href="#waitlist" className="hover:text-white">Contacto</a>
          </div>
        </div>
      </footer>
    </div>
