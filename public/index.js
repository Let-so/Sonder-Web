...
import { HeartPulse, ShieldCheck, Sparkles, Stethoscope, Bell, FileUp, Watch, Pill, Bot, ChevronRight, Menu } from "lucide-react";

// =====================================================
//  MOBILE-FIRST LANDING • SONDER
//  - Tono ultra moderno
//  - Logo con línea fina centrada y desplazada levemente a la derecha
//  - Optimizaciones para móvil (tipos, espacios, animaciones discretas)
// =====================================================
...
const Section = ({ id, eyebrow, title, subtitle, children }) => (
  <section id={id} className="relative py-14 sm:py-20">
    <Container>
      <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="mb-8 sm:mb-10">
        {eyebrow && (
          <div className="mb-3">
            <Badge className="rounded-full px-3 py-1 text-[10px] sm:text-xs tracking-wider">{eyebrow}</Badge>
          </div>
        )}
        {title && (
          <h2 className="text-2xl/tight sm:text-4xl/tight font-semibold tracking-tight">{title}</h2>
        )}
        {subtitle && (
          <p className="mt-3 max-w-3xl text-sm sm:text-base text-muted-foreground">{subtitle}</p>
        )}
      </motion.div>
      {children}
    </Container>
...
// —— Fondo dinámico (ligero en mobile) ——
const Bg = () => (
  <div aria-hidden className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
    {/* Gradiente principal */}
    <div className="absolute inset-0 bg-[radial-gradient(40%_30%_at_50%_10%,theme(colors.blue.500/.22),transparent),radial-gradient(25%_20%_at_90%_10%,theme(colors.fuchsia.500/.22),transparent),radial-gradient(30%_25%_at_10%_10%,theme(colors.cyan.400/.3),transparent)]" />
    {/* Malla sutil (sólo >= sm) */}
    <div className="hidden sm:block absolute inset-0 [background:linear-gradient(to_right,theme(colors.white/10)_1px,transparent_1px),linear-gradient(to_bottom,theme(colors.white/10)_1px,transparent_1px)] [background-size:40px_40px] opacity-10" />
  </div>
);

// —— LOGO SONDER ——
// Representación del logo con la línea fina centrada y desplazada unos px a la derecha.
// Para reemplazar por PNG/SVG final, pasá src al componente <LogoS src="/ruta/logo.png" />
  ...
  <header className="sticky top-0 z-40 backdrop-blur supports-[backdrop-filter]:bg-background/70 border-b">
    <Container className="flex h-14 items-center justify-between gap-2">
      ...
      <div className="flex items-center gap-1 sm:gap-2">
        ...
        <button className="md:hidden p-2 text-muted-foreground" aria-label="Abrir menú"><Menu className="h-5 w-5" /></button>
      </div>
    </Container>
  </header>
);
      ...
      <div className="grid items-center gap-8 lg:grid-cols-2">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          ...
          <h1 className="mt-4 text-3xl/tight sm:text-5xl/tight font-semibold tracking-tight">
            Menos <span className="bg-gradient-to-r from-blue-600 to-fuchsia-600 bg-clip-text text-transparent">ensayo y error</span>,
            <br /> más vida.
          </h1>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6 }} className="relative">
          <div className="relative mx-auto w-full max-w-sm">
            <Card className="rounded-3xl border-0 bg-gradient-to-b from-background/60 to-background/90 shadow-2xl backdrop-blur">
              <CardHeader className="pb-2">
                <CardTitle className="flex items-center gap-2 text-sm sm:text-base"><HeartPulse className="h-5 w-5 text-blue-600" /> Panel del paciente</CardTitle>
              </CardHeader>
...
const TinyStat = ({ label, value, suffix }) => (
  <div className="rounded-2xl border bg-background/60 p-3 text-center">
    <div className="text-xl sm:text-2xl font-semibold tracking-tight">{value}{suffix && <span className="text-sm sm:text-base font-medium">{suffix}</span>}</div>
...
// —— Secciones ——
const QueEs = () => (
  <Section id="que-es" eyebrow="Qué es Sonder" title={<>
    Una plataforma de salud que entiende tu particularidad
  </>} subtitle={"Sonder conecta al médico con el paciente mediante un código único y crea un espacio compartido para registrar síntomas, hábitos, medicación y estudios. Con IA, detecta tendencias y alerta al profesional si algo no va bien."}>
    <div className="grid gap-4 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <FeatureCard icon={<Watch className="h-5 w-5" />} title="Datos fisiológicos" text="Integración con smartwatches (p. ej., Xiaomi Mi Band) para pasos, ritmo cardíaco y más." />
      ...
      <FeatureCard icon={<Bot className="h-5 w-5" />} title="Asistencia con IA" text="Un asistente conversa contigo y organiza la información para el seguimiento clínico." />
      <FeatureCard icon={<Bell className="h-5 w-5" />} title="Alertas tempranas" text="Cuando la respuesta a un tratamiento no parece adecuada, la IA notifica al médico con explicación basada en datos." />
    </div>
  </Section>
);
...
const FeatureCard = ({ icon, title, text }) => (
  <motion.div initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.45 }}>
    <Card className="h-full rounded-2xl border bg-background/60 backdrop-blur">
      <CardHeader className="pb-2">
        <CardTitle className="flex items-center gap-2 text-[15px] font-semibold">{icon} {title}</CardTitle>
      </CardHeader>
      <CardContent>
...
const ComoFunciona = () => (
  <Section id="como-funciona" eyebrow="Cómo funciona" title={<>
    Del consultorio a los datos, sin fricción
    ...
    <ol className="grid gap-3 sm:gap-4 sm:grid-cols-2 lg:grid-cols-4">
      <Step num={1} title="Código del médico" text="El profesional crea un código desde su panel y se lo comparte al paciente." />
      <Step num={2} title="Registro del paciente" text="La persona se registra, ingresa el código y completa preguntas personalizadas según su patología." />
      <Step num={3} title="Seguimiento" text="El paciente sube estudios, registra medicación y, si quiere, conecta su smartwatch." />
      <Step num={4} title="Detección temprana" text="La IA busca patrones y puede alertar al médico si el tratamiento no parece funcionar como se espera." />
    </ol>
    <p className="mt-5 text-[11px] sm:text-xs text-muted-foreground">Nota: Sonder no prescribe ni recomienda medicamentos. Es una herramienta de apoyo para el equipo de salud.</p>
  </Section>
);
...
const Step = ({ num, title, text }) => (
  <li>
    <div className="relative h-full rounded-2xl border bg-background/60 p-4 sm:p-5 backdrop-blur">
      <div className="absolute -top-3 -left-3 grid h-8 w-8 place-items-center rounded-xl bg-gradient-to-br from-blue-600 to-fuchsia-600 text-white text-sm font-semibold shadow-md">{num}</div>
...
const DeepFeature = ({ icon, title, bullets }) => (
  <Card className="rounded-2xl border bg-background/60">
    ...
    <CardContent>
      <ul className="space-y-2 text-sm text-muted-foreground">
        {bullets.map((b, i) => (<li key={i} className="flex items-start gap-2"><span className="mt-1 inline-block h-1.5 w-1.5 rounded-full bg-gradient-to-r from-blue-600 to-fuchsia-600" /> {b}</li>))}
      </ul>
    </CardContent>
  </Card>
);
...
const DemoWaitlist = () => {
  ...
  return (
    <Section id="waitlist" eyebrow="Probá Sonder" title="Solicitá una demo o sumate a la lista de espera" subtitle="Dejanos tu contacto y te escribimos cuando esté disponible para tu perfil.">
      <div className="grid gap-6 sm:gap-8 lg:grid-cols-2">
        <Card className="rounded-3xl border bg-background/60">
          <CardHeader className="pb-2"><CardTitle className="text-[15px] sm:text-base">Unite a la lista</CardTitle></CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="grid gap-3 sm:gap-4">
              ...
              <div className="grid gap-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <Button type="button" variant={role==="Paciente"?"default":"outline"} className={role!=="Paciente"?"bg-transparent":" bg-gradient-to-r from-blue-600 to-fuchsia-600 text-white"} onClick={()=>setRole("Paciente")}>Paciente</Button>
                  <Button type="button" variant={role==="Médico"?"default":"outline"} className={role!=="Médico"?"bg-transparent":" bg-gradient-to-r from-blue-600 to-fuchsia-600 text-white"} onClick={()=>setRole("Médico")}>Médico</Button>
                </div>
              </div>
              <div className="flex items-center justify-between text-[11px] sm:text-xs text-muted-foreground">
                <div className="flex items-center gap-2"><Switch id="updates" /><Label htmlFor="updates">Recibir novedades del proyecto</Label></div>
                <span className="">{role}</span>
              </div>
              <Button type="submit" className="h-10 bg-gradient-to-r from-blue-600 to-fuchsia-600 text-white">Enviar</Button>
              {sent && (
                <p className="text-sm text-green-600">¡Listo! Te contactaremos pronto. (Demo de envío)</p>
              )}
            </form>
          </CardContent>
        </Card>
        <Card id="demo" className="rounded-3xl border bg-background/60">
          <CardHeader className="pb-2"><CardTitle className="text-[15px] sm:text-base">Solicitar demo para instituciones</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            <p className="text-sm text-muted-foreground">¿Sos parte de un equipo de salud? Coordinemos una demo privada enfocada en tus procesos.</p>
            <ul className="text-sm text-muted-foreground space-y-2">
              <li className="flex items-start gap-2"><span className="mt-1 inline-block h-1.5 w-1.5 rounded-full bg-gradient-to-r from-blue-600 to-fuchsia-600" /> Configuración por especialidad</li>
              <li className="flex items-start gap-2"><span className="mt-1 inline-block h-1.5 w-1.5 rounded-full bg-gradient-to-r from-blue-600 to-fuchsia-600" /> Perfiles de acceso y seguridad</li>
              <li className="flex items-start gap-2"><span className="mt-1 inline-block h-1.5 w-1.5 rounded-full bg-gradient-to-r from-blue-600 to-fuchsia-600" /> Exportación e interoperabilidad</li>
            </ul>
...
const FAQ = () => (
  <Section id="faq" eyebrow="Preguntas frecuentes" title="Lo esencial, claro y directo">
    <div className="grid gap-3 sm:gap-4 sm:grid-cols-2">
      ...
      <FaqItem q="¿Qué datos usa la IA?" a="Combina información declarada por el paciente (síntomas, hábitos, adherencia) con estudios y, si lo eligen, datos de smartwatch." />
      <FaqItem q="¿Qué pasa con mi privacidad?" a="Priorizamos la seguridad de los datos, con cifrado en tránsito y reposo y controles de acceso. Diseñado para adecuación normativa en Argentina." />
    </div>
  </Section>
);
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-[11px] text-muted-foreground">© {new Date().getFullYear()} — Todos los derechos reservados</div>
          </div>
        </div>
